...
from agent.main_agent_mock_lab8 import MockAgent as MainAgent
...
async def run_benchmark_with_results(agent_version: str, dataset_path: str):
    print(f"\n🚀 Khởi động Benchmark cho {agent_version} (MOCK)...")
    ...
    if not os.path.exists(dataset_path):
        print(f"❌ Thiếu {dataset_path}. Hãy chạy 'python data/synthetic_gen_lab8.py' trước.")
        return None, None
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="AI Evaluation Factory Benchmark Runner (Mock)")
    parser.add_argument("--dataset", type=str, default="data/golden_set_lab8.jsonl", help="Đường dẫn đến file golden_set.jsonl")
    args = parser.parse_args()

    # Chạy benchmark cho phiên bản Mock
    results, summary = await run_benchmark_with_results("Agent_Mock_Weak", args.dataset)
    ...
    if summary:
        ...
        with open("reports/summary_mock_lab8.json", "w", encoding="utf-8") as f:
            json.dump(summary, f, ensure_ascii=False, indent=2)
        with open("reports/benchmark_results_mock_lab8.json", "w", encoding="utf-8") as f:
